...
class Promise {
    ...
    class WsPromiseException(message:String): Exception(message){
        companion object{
            ...
            fun makeException(error:Any?):WsPromiseException{
    ...
    companion object{
        ...
        fun all(list:ArrayList<Promise?>):Promise{
            val p = Promise()
            if (list.size > 0){
                android.os.Handler().post(object :Runnable{
                    override fun run() {
                        ...
                        for (obj in list){
                            val promise = obj!!
                            promise.then {
                                result.set(promise.tag as Int, it ?: Null())
                                completedCount += 1
        ...
        fun allmap(hash:JSONObject):Promise{
            val p = Promise()
            if (hash.length() > 0){
                android.os.Handler().post(object :Runnable{
                    override fun run() {
    ...
    private var _state:WsPromiseState
     ...
     */
    constructor(){
        _state = WsPromiseState.WsPromiseStatePending
        _resolve_callbacks = JSONArray()
        _reject_callbacks = JSONArray()
        _value = null
    }
     ...
     * (public) then操作
     */
    public fun then(onResolved:(obj:Any?)->Any?):Promise{
     ...
     * (public) catch操作
     */
    public fun error(onRejected:(obj:Any?)->Unit):Promise{
     ...
     * (public) error的别名
     */
    public fun _catch(onRejected:(obj:Any?)->Unit):Promise{
